...
ADMIN_EMAIL="admin@email.com" For SSL certificates
MYSQL_ROOT_PASS="StrongPass!" 
 # Change default credentials! 
 ##🌟 Features
  Automatic Stack Deployment: Nginx (optimized workers + security headers) 
  PHP-FPM (Opcache + 256M memory) 
  MariaDB (utf8mb4 + hardening)
SSL Options: Auto-configured 
Self-Signed (dev) Let's Encrypt (production)
WordPress: Secure wp-config.php 
...
-----------------------------------------------
Note.
 Some precise Errors from the Wordpress installer could be not a real error.
